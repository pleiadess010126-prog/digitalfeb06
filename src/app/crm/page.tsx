                    ...
                    <div className="flex justify-between items-start mb-2">
                        <div className="flex items-center gap-2">
                            {lead.socialProfiles?.[0]?.profilePictureUrl ? (
                                <img src={lead.socialProfiles[0].profilePictureUrl} alt={lead.name || 'Lead'} className="w-6 h-6 rounded-full" />
                            ) : (
                                <div className="w-6 h-6 rounded-full bg-indigo-100 text-indigo-600 flex items-center justify-center text-xs font-bold">
                                    {(lead.name || '?').charAt(0)}
                                </div>
                            )}
                            <span className="font-medium text-slate-800 text-sm truncate max-w-[120px]">{lead.name || 'Unknown'}</span>
                        </div>
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {leads.slice(0, 5).map(lead => (
                                        <tr key={lead.id} className="hover:bg-slate-50 transition-colors">
                                            <td className="px-6 py-4 font-medium text-slate-900">{lead.name || 'Unknown'}</td>
