...
import GlobalBridgeMonitor from '@/components/GlobalBridgeMonitor';
import RestrictedMarketBridge from '@/components/RestrictedMarketBridge';
import EngagementHub from '@/components/EngagementHub';
import BrandSafetyGuard from '@/components/BrandSafetyGuard';
import SystemIntegrityMonitor from '@/components/SystemIntegrityMonitor';
import TranscreatedCampaignWizard from '@/components/TranscreatedCampaignWizard';
...
import {
    ...
    Video, Languages, Clock, FlaskConical, Link2, Building2, Shield, ShieldCheck, CreditCard, Wrench, Database,
    Scissors, Hash, Target, CheckCircle2, ImageIcon, Wand2, Repeat2, Swords, MessageSquare, MessageCircle, Gift, Crown, Send
} from 'lucide-react';
import type { ContentItem, Campaign } from '@/types';

type TabType = 'overview' | 'content' | 'analytics' | 'automation' | 'leads' | 'collaboration' | 'localization' | 'tools' | 'settings' | 'knowledge' | 'engagement' | 'safety';
...
function DashboardContent() {
    ...
    const [showTranscreatedWizard, setShowTranscreatedWizard] = useState(false);
    ...
    const [automationTab, setAutomationTab] = useState<'overview' | 'content' | 'campaigns' | 'war-room'>('overview');
    const [toolsTab, setToolsTab] = useState<string>('influencers');
    const [showUserMenu, setShowUserMenu] = useState(false);
    const [currentLanguage, setCurrentLanguage] = useState('en');
    const [organizationId, setOrganizationId] = useState<string>('demo_org_123');
    ...
    useEffect(() => {
        ...
        if (tab && ['overview', 'content', 'analytics', 'automation', 'leads', 'collaboration', 'localization', 'tools', 'settings', 'knowledge', 'engagement', 'safety'].includes(tab)) {
            // eslint-disable-next-line react-hooks/set-state-in-effect
            setActiveTab(tab);
        }
    }, [searchParams]);
    ...
    // Fetch Real Analytics (Gap Closure)
    const [realStats, setRealStats] = useState<any>(null);
    useEffect(() => {
        const fetchStats = async () => {
            try {
                const endpoint = `/api/analytics/summary?organizationId=${organizationId || 'demo_org_123'}`;
                const res = await fetch(endpoint).catch(e => {
                    console.error("Network Error in fetchStats:", e);
                    return null;
                });
                ...
                console.warn("Gracefully handled fetchStats failure:", err);
            }
        };
        fetchStats();
    }, [organizationId]);

    // Update campaign stats with real data if available
    useEffect(() => {
        if (realStats) {
            setCampaign(prev => ({
                ...prev,
                stats: {
                    ...prev.stats,
                    totalContent: realStats.totalContent,
                    publishedThisMonth: realStats.publishedThisMonth,
                    organicTraffic: realStats.organicTraffic,
                    avgDwellTime: realStats.avgDwellTime.toString(),
                }
            }));
        }
    }, [realStats]);
    ...
    const tabs = [
        ...
        { id: 'engagement' as TabType, label: 'Engagement', icon: MessageCircle, color: 'bg-pink-600' },
        ...
        { id: 'safety' as TabType, label: 'Brand Safety', icon: ShieldCheck, color: 'bg-red-600' },
        { id: 'knowledge' as TabType, label: 'Brand Vault', icon: Database, color: 'bg-indigo-600' },
        { id: 'leads' as TabType, label: 'Sales CRM', icon: Target, color: 'bg-emerald-600' },
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50">
            ...
            <header className="bg-[#0a0a0f] text-white px-8 h-20 flex items-center relative z-[100]">
                <div className="flex items-center justify-between w-full">
                    <ProductTour />
                    <div className="flex items-center gap-6">
                        <Logo size="md" darkMode={true} showTagline={false} linkToHome={true} />
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="relative">
                            ...
                            <div id="tour-profile" className="absolute top-0 left-0 w-full h-full pointer-events-none" />
                                ...
                                <div className="absolute right-0 top-full mt-2 w-64 bg-white rounded-xl shadow-2xl border border-slate-200 overflow-hidden z-[100]">
                                    ...
                                    <div className="p-2">
                                        ...
                                        <button
                                            ...
                                            className="w-full flex items-center gap-3 px-3 py-2 text-pink-600 hover:bg-pink-50 rounded-lg transition-colors text-left"
                                        >
                                            <MessageCircle className="w-4 h-4" />
                                            <span className="text-sm font-medium">Engagement Hub</span>
                                        </button>
                                        <button
            ...
            <div className="bg-white border-b border-slate-200 shadow-sm sticky top-0 z-50">
                <div className="px-8" id="tour-sidebar">
                    <div className="flex gap-1 overflow-x-auto scrollbar-hide no-scrollbar">
                        {tabs.map((tab) => (
                            ...
                            >
                                ...
                                {tab.id === 'automation' && <div id="tour-automation" className="absolute inset-0 pointer-events-none" />}
                                {tab.id === 'localization' && <div id="tour-localization" className="absolute inset-0 pointer-events-none" />}
                            </button>
                        ))}
                    </div>
                </div>
            ...
            <div className="px-6 py-4">
                ...
                {activeTab === 'overview' && (
                    <div className="space-y-3">
                        {/* Key Metrics Row */}
                        <div className="grid grid-cols-4 gap-3" id="tour-stats">
                            {[
                                ...
                                { label: t('engagement'), value: '4.8%', change: '+0.5%', icon: Activity, gradient: 'from-amber-500 to-orange-600', tab: 'engagement' as TabType },
                            ].map((metric, idx) => (
                        ...
                        {/* Global Launch Spotlight - NEW */}
                        <button
                            onClick={() => setActiveTab('localization')}
                            className="bg-gradient-to-r from-indigo-900 via-violet-900 to-fuchsia-900 rounded-2xl p-6 text-left shadow-xl shadow-indigo-500/20 group relative overflow-hidden"
                        >
                            {/* Background Pattern */}
                            <div className="absolute inset-0 opacity-10 pointer-events-none">
                                <Globe className="w-64 h-64 -right-16 -bottom-16 absolute text-white" />
                            </div>
                                ...
                                <div className="space-y-4 max-w-2xl">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="px-3 py-1 rounded-full bg-emerald-500/20 text-emerald-400 text-[10px] font-bold uppercase tracking-widest border border-emerald-500/30">
                                            LIVE: GLOBAL LAUNCH 2026
                                        </span>
                                    </div>
                                    <div>
                                        <h3 className="text-2xl font-black text-white mb-2">Expanding Your Reach Globally</h3>
                                        <p className="text-white/70 text-sm leading-relaxed">
                                            The DigitalMEng Marketing Engine is currently pushing localized promotional content across 20+ markets.
                                            Verified targets: <b>English, Spanish, Hindi, Chinese, and Arabic.</b>
                                        </p>
                                    </div>
                                    <div className="flex items-center gap-4">
                                        <div className="flex -space-x-2">
                                            {[1, 2, 3, 4, 5].map(i => (
                                                <div key={i} className="w-8 h-8 rounded-full border-2 border-indigo-900 bg-slate-800 flex items-center justify-center text-[10px] font-bold text-white">
                                                    {['US', 'ES', 'IN', 'CN', 'AE'][i - 1]}
                                                </div>
                                            ))}
                                        </div>
                                        <span className="text-white/60 text-xs font-medium">+15 more markets active</span>
                                    </div>
                                </div>

                                {/* Visual Asset */}
                                <div className="hidden lg:block w-48 h-32 rounded-xl overflow-hidden border-2 border-white/20 shadow-2xl skew-x-3 -rotate-3 transition-transform group-hover:rotate-0 group-hover:skew-x-0">
                                    <Image
                        ...
                        <div className="grid grid-cols-12 gap-3">
                            ...
                            {/* GEO Scorecard Panel */}
                            <div className="col-span-3 bg-white rounded-xl p-5 shadow-sm border border-slate-100 flex flex-col justify-between" id="tour-geoscore">
                                <div>
                                    <div className="flex items-center justify-between mb-2">
                                        <h3 className="font-bold text-slate-800">GEO Scorecard</h3>
                                        <div className="p-1.5 rounded-lg bg-violet-100 text-violet-600">
                                            <Target className="w-4 h-4" />
                                        </div>
                                    </div>
                                    <div className="flex items-end gap-2">
                                        <span className="text-4xl font-black text-slate-900 leading-none">84.5</span>
                                        <span className="text-lg font-bold text-slate-400">/100</span>
                                    </div>
                                    <p className="text-xs text-slate-500 mt-2">Generative Engine Optimization (GEO) index. Your content is highly cited.</p>
                                </div>
                                <div className="mt-4 pt-4 border-t border-slate-50">
                                    ...
                                    <div className="flex gap-1 h-2">
                                        <div className="flex-[0.7] bg-violet-500 rounded-full" />
                                        <div className="flex-[0.2] bg-blue-500 rounded-full" />
                                        <div className="flex-[0.1] bg-slate-200 rounded-full" />
                                    </div>
                                </div>
                            </div>
                ...
                {/* Localization Tab */}
                {
                    activeTab === 'localization' && (
                        <div className="space-y-8">
                            <button
                                onClick={() => setShowTranscreatedWizard(true)}
                                className="w-full p-8 rounded-[3rem] bg-gradient-to-r from-violet-600 via-indigo-600 to-blue-600 text-white shadow-2xl shadow-violet-500/20 hover:scale-[1.01] transition-all group relative overflow-hidden"
                            >
                                ...
                                <div className="relative z-10 flex items-center justify-between">
                                    <div className="text-left space-y-2">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <h2 className="text-3xl font-black">Launch Global Blast</h2>
                                        </div>
                                        <p className="text-white/80 text-lg font-medium max-w-xl">
                                            Deploy AI-transcreated campaigns to <strong>China, Russia, and Global TikTok</strong> through our secure tunneling network.
                                        </p>
                                    </div>
                                    <div className="flex items-center gap-4">
                                        <div className="text-right">
                                            <p className="text-2xl font-black text-white">3 Nodes</p>
                                            <p className="text-white/60 text-sm font-bold uppercase tracking-widest">Active & Ready</p>
                                        </div>
                    ...
                    activeTab === 'automation' && (
                        <div className="space-y-6">
                            ...
                            <div className="bg-gradient-to-r from-violet-600 via-purple-600 to-indigo-600 rounded-2xl p-6 text-white shadow-xl">
                                ...
                                {/* Automation Sub-tabs */}
                                <div className="flex gap-2 mt-6 p-1 bg-black/10 rounded-xl w-fit">
                                    {[
                                        { id: 'overview', label: 'Overview', icon: LayoutDashboard },
                                        { id: 'content', label: 'Content AI', icon: FileText },
                                        { id: 'campaigns', label: 'Campaigns', icon: Mail },
                                        { id: 'war-room', label: 'Agent War Room', icon: Swords }
                                    ].map((tab) => (
                                        <button
                                            key={tab.id}
                                            onClick={() => setAutomationTab(tab.id as any)}
                                            className={`px-4 py-2 rounded-lg text-sm font-bold transition-all flex items-center gap-2 ${automationTab === tab.id
                                    ...
                                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                        ...
                                        <div className="bg-white rounded-2xl border-2 border-blue-200 shadow-sm overflow-hidden">
                                            ...
                                            <div className="p-4 space-y-4">
                                                ...
                                                <button
                                                    onClick={() => setAutomationTab('content')}
                                        ...
                                        <div className="bg-white rounded-2xl border-2 border-emerald-200 shadow-sm overflow-hidden">
                                            <div className="bg-gradient-to-r from-emerald-500 to-teal-500 p-4 text-white">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <h3 className="font-bold text-lg">⚔️ Agent War Room</h3>
                                                        <p className="text-sm text-white/80">Multi-agent collaborative strategy</p>
                                                    </div>
                                                </div>
                                            </div>
                                            <div className="p-4 space-y-4">
                                                <div className="text-center py-4 bg-emerald-50 rounded-xl">
                                                    ...
                                                    <p className="text-xs text-slate-500 mt-1">
                                                        Strategists, Copywriters & Critics<br />
                                                        debate your best content ideas
                                                    </p>
                                                </div>
                                                <button
                                                    onClick={() => setAutomationTab('war-room')}
                            ...
                            {automationTab === 'campaigns' && (
                                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                                    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-12 text-center">
                                        ...
                                        <h2 className="text-2xl font-bold text-slate-800 mb-2">Campaign Autopilot Center</h2>
                                        <p className="text-slate-500 max-w-lg mx-auto mb-8">
                                            Your automated email and SMS campaigns are being managed by our AI engine.
                                            Click below to open the dedicated campaign builder.
                                        </p>
                                        <button
                                            onClick={() => router.push('/autopilot')}
                                            className="px-8 py-3 bg-rose-600 text-white rounded-xl font-bold hover:bg-rose-700 shadow-lg shadow-rose-500/20 transition-all"
                                        >
                        ...
                        <div className="flex gap-6 min-h-[600px]">
                                    ...
                                    <nav className="p-2 space-y-4">
                                        {[
                                            ...
                                            },
                                            {
                                                group: t('system'),
                                                items: [
                                                    { id: 'providers', label: 'AI Providers', icon: Sparkles, color: 'text-violet-600', bg: 'bg-violet-50' },
                            ...
                            <div className="flex-1 space-y-6">
                                ...
                                {settingsTab === 'brand' && (
                                    <div className="bg-slate-900 rounded-2xl p-6 shadow-xl">
                                        <BrandProfileSettings
                                            key={organizationId || 'demo_org_123'}
                                            organizationId={organizationId || 'demo_org_123'}
                                ...
                                {settingsTab === 'credentials' && (
                                    <CredentialsSettings organizationId={organizationId || 'demo_org_123'} />
                                )}
                ...
                {
                    showTranscreatedWizard && (
                        <TranscreatedCampaignWizard
                            onClose={() => setShowTranscreatedWizard(false)}
                        />
                    )}
            </div>
        </div>
