...
import AdminApiProviders from '@/components/AdminApiProviders';
...
export default function AdminPage() {
    ...
    const [settingsSubTab, setSettingsSubTab] = useState<string>('all');
    ...
    // API Key States - Expanded with many providers
    ...
    // Test Status States - All providers
    const [testResults, setTestResults] = useState<Record<string, ApiTestResult>>({
        // Text
        openai: { status: 'idle' }, anthropic: { status: 'idle' }, gemini: { status: 'idle' },
        groq: { status: 'idle' }, together: { status: 'idle' }, cohere: { status: 'idle' },
        openrouter: { status: 'idle' }, ollama: { status: 'idle' },
        // Video
        did: { status: 'idle' }, synthesia: { status: 'idle' }, pictory: { status: 'idle' },
        heygen: { status: 'idle' }, runway: { status: 'idle' },
        // Voice
        elevenlabs: { status: 'idle' }, playht: { status: 'idle' }, murf: { status: 'idle' },
        googleTts: { status: 'idle' }, azureTts: { status: 'idle' },
        // Image
        stabilityai: { status: 'idle' }, leonardo: { status: 'idle' },
        replicate: { status: 'idle' }, clipdrop: { status: 'idle' },
        // SMS
        twilio: { status: 'idle' }, msg91: { status: 'idle' },
        textlocal: { status: 'idle' }, plivo: { status: 'idle' },
        // Email
        sendgrid: { status: 'idle' }, resend: { status: 'idle' },
        mailgun: { status: 'idle' }, postmark: { status: 'idle' }, ses: { status: 'idle' },
        // Social
        metaAppId: { status: 'idle' }, metaAppSecret: { status: 'idle' },
        youtubeApiKey: { status: 'idle' }, tiktokKey: { status: 'idle' },
        wechatAppId: { status: 'idle' }, vkToken: { status: 'idle' },
        // Payment
                ...
                <main className="flex-1 p-6 overflow-auto">
                    ...
                    {activeTab === 'settings' && (
                        <div className="space-y-6">
                            ...
                            <div className="flex flex-wrap gap-2 border-b border-slate-700 pb-4">
                                {[
                                    ...
                                    <button
                                        ...
                                        className={`px-4 py-2 text-sm rounded-lg transition-colors ${settingsSubTab === tab.id
                            ...
                            {(settingsSubTab === 'all' || settingsSubTab === 'aws') && (
                                <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6">
                            ...
                            {/* AI Content Creation Section - Expanded 30+ Providers */}
                            {(settingsSubTab === 'all' || settingsSubTab === 'ai') && (
                                <AdminApiProviders
                                    apiKeys={apiKeys}
                            ...
                            {(settingsSubTab === 'all' || settingsSubTab === 'translation') && (
                                <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6">
                                    <div className="flex items-center gap-3 mb-6">
                            ...
                            {(settingsSubTab === 'all' || settingsSubTab === 'appconfig') && (
                                <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6">
                                    <div className="flex items-center gap-3 mb-6">
                            ...
                            {(settingsSubTab === 'all' || settingsSubTab === 'analytics') && (
                                <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6">
                                    <div className="flex items-center gap-3 mb-6">
