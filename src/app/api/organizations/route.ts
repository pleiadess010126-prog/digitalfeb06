...
export async function GET(request: NextRequest) {
    try {
        ...
        if (orgId) {
            let organization = await db.getOrganization(orgId);

            // Auto-create demo organization if it doesn't exist
            if (!organization && orgId === 'demo_org_123') {
                console.log('🏗️ Auto-creating demo organization');
...
export async function PATCH(request: NextRequest) {
    try {
        ...
        // For demo org, the server can't persist to localStorage, so we just return success
        // to let the client-side localStorage handling take care of it.
        if (id === 'demo_org_123') {
            return NextResponse.json({
                id,
                ...updates,
                message: 'Demo mode: Changes persisted to client-side storage only.'
            });
        }
