...
type ServiceType = 'openai' | 'anthropic' | 'groq' | 'did' | 'synthesia' | 'pictory' | 'elevenlabs' | 'stabilityai';
...
async function testGroq(apiKey: string): Promise<TestResult> {
    try {
        // Groq uses OpenAI-compatible API, so we can list models to validate the key
        ...
        if (response.ok) {
            ...
            return {
                success: false,
                message: error.error?.message || `Groq API error: ${response.status}`,
            };
        }
    } catch (error) {
        return {
            success: false,
            message: `Failed to connect to Groq: ${error instanceof Error ? error.message : 'Unknown error'}`,
        };
    }
}
