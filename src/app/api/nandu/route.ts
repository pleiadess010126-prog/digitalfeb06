...
let groqClient: OpenAI | null = null; // Groq uses OpenAI SDK
...
async function chatWithGroq(messages: ChatMessage[]): Promise<string> {
    ...
    return response.choices[0].message.content || 'I apologize, I had trouble generating a response via Groq. Please try again.';
}
...
export async function GET() {
    ...
    return NextResponse.json({
        ...
        mode: groqClient || openaiClient || anthropicClient ? 'ai' : 'simulation',
    });
}
