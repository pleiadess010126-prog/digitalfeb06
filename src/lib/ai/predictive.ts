...
export class PredictiveEngine {
    ...
    private async getEngagementHeatmap(contactId?: string): Promise<EngagementData[]> {
        // Try to fetch real engagement data from ContentItem performance metrics
        try {
            const historicalContent = await prisma.contentItem.findMany({
                where: {
            ...
            if (historicalContent.length > 10) { // Threshold for meaningful real data
                const heatmap: Record<string, EngagementData> = {};
                ...
                // Aggregate real data
                for (const item of historicalContent) {
                    ...
                    const slot = heatmap[key];
                    slot.count++;
                    // Normalize views/engagement to a 0-1 range for the "rate" metrics
                    // This is a simplified mapping: engagement/views = clickRate-like metric
                    const engagementRate = item.views > 0 ? item.engagement / item.views : 0;

                    slot.openRate += (item.views > 100 ? 0.4 : 0.2); // Proxy for "reach"
                    slot.clickRate += engagementRate;
                    slot.responseRate += (engagementRate * 0.1);
                }

                // Average out the rates
                return Object.values(heatmap).map(slot => ({
                    ...slot,
                    openRate: slot.count > 0 ? slot.openRate / slot.count : 0,
                    clickRate: slot.count > 0 ? slot.clickRate / slot.count : 0,
                    responseRate: slot.count > 0 ? slot.responseRate / slot.count : 0,
                }));
            }
        } catch (error) {
            console.error('Failed to fetch real engagement data, falling back to simulation:', error);
        }

        // Fallback: Simulated engagement patterns
