...
export interface GenerateContentParams {
    ...
    byoConfig?: BYOConfig; // Dynamic hosting configuration
    refinementFeedback?: string; // Optional feedback for self-healing loops
    regionalContext?: string; // Information about target region/culture for localization
}

export interface BYOConfig {
    strategy: 'managed' | 'byo';
    privateCloudUrl?: string;
...
/**
 * Detect which AI provider is available (Global platforms)
 */
export function getAvailableProvider(byoConfig?: BYOConfig): AIProvider {
    // If BYO is active, prioritize the provider defined in their config
    if (byoConfig?.strategy === 'byo' && byoConfig.credentials) {
        if (byoConfig.credentials.openai?.apiKey) return 'openai';
        if (byoConfig.credentials.anthropic?.apiKey) return 'anthropic';
        if (byoConfig.credentials.google_gemini?.apiKey) return 'openai'; // Gemini often uses OpenAI compatible wrappers or direct
    }
...
/**
 * Enhanced OpenAI Generator for BYO Support
 */
async function generateWithOpenAIByo(params: GenerateContentParams, byoConfig: BYOConfig): Promise<GeneratedContent> {
    const userApiKey = byoConfig.credentials?.openai?.apiKey || byoConfig.credentials?.google_gemini?.apiKey;
    const baseURL = byoConfig.privateCloudUrl || undefined;

    if (!userApiKey) {
        return generateWithOpenAI(params); // Fallback to platform if user key missing but route allowed
    }

    const tempClient = new OpenAI({
        apiKey: userApiKey,
        baseURL: baseURL, // Point to private cloud if provided
    });

    const prompt = buildPrompt(params);

    const response = await tempClient.chat.completions.create({
        model: byoConfig.credentials?.openai?.model || 'gpt-4-turbo-preview',
        messages: [{ role: 'user', content: prompt }],
        max_tokens: params.length === 'short' ? 500 : params.length === 'long' ? 2000 : 1000,
        temperature: 0.7,
    });

    return parseGeneratedContent(response.choices[0].message.content || '', params);
}
...
function buildPrompt(params: GenerateContentParams, ragContext: string = ''): string {
    const { topic, keywords, contentType, targetAudience, tone = 'professional', length = 'medium', language = 'en', refinementFeedback, regionalContext } = params;
    ...
    const regionalInstruction = regionalContext ? `
REGIONAL & CULTURAL LOCALIZATION:
Target Region/Culture: ${regionalContext}
- Adapt metaphors, idioms, and tone to resonate with local audience in ${regionalContext}.
- Ensure visual descriptions and examples are culturally relevant.
` : '';
...
` : '';
    ...
    const refinementInstruction = refinementFeedback ? `
CRITICAL - REFINEMENT NEEDED:
Your previous output was analyzed and found to have the following issues:
${refinementFeedback}

Please RE-GENERATE the content addressing these specific points. Ensure the new version is significantly improved.
` : '';
 ...
 * Scoped by hosting strategy: Managed vs BYO
 */
export async function generateContent(params: GenerateContentParams, provider?: AIProvider): Promise<GeneratedContent> {
    const byo = params.byoConfig;
    const selectedProvider = provider || getAvailableProvider(byo);

    console.log(`Generating content with provider: ${selectedProvider} [Strategy: ${byo?.strategy || 'managed'}]`);

    try {
        // If BYO strategy is active and we're using OpenAI/Gemini/private cloud
        if (byo?.strategy === 'byo' && selectedProvider === 'openai') {
            return await generateWithOpenAIByo(params, byo);
        }
        ...
        console.error(`Error with ${selectedProvider} [${byo?.strategy}], falling back to mock:`, error);
        return generateMockContent(params);
    }
}
