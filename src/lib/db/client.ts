...
export class LocalStorageDatabase implements DatabaseClient {
    ...
    async updateOrganization(id: string, updates: Partial<Organization>): Promise<Organization> {
        const organizations = getStorage<Organization>('organizations');
        let index = organizations.findIndex(o => o.id === id);

        if (index === -1) {
            // Auto-create if not found (useful for demo/dev mode)
            const newOrg: Organization = {
                id,
                name: updates.name || 'Organization',
                slug: (updates.name || 'org').toLowerCase().replace(/\s+/g, '-'),
                ownerId: 'demo_user',
        ...
        // Merge settings if they exist in updates
        const currentOrg = organizations[index];
        const newSettings = updates.settings
            ? { ...currentOrg.settings, ...updates.settings }
            : currentOrg.settings;
...
function getDatabaseClient(): DatabaseClient {
    ...
    if (isServer && hasDatabaseUrl) {
        try {
            ...
            if (prismaDb) return prismaDb;
        } catch (e) {
            console.error('⚠️ Failed to initialize Prisma Client, falling back to LocalStorage:', e);
        }
    }
