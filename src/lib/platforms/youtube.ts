    ...
    async uploadVideo(video: YouTubeVideo, videoFile: Blob): Promise<YouTubeResponse> {
        try {
            ...
            if (!response.ok) {
                ...
                let errorMessage = error.error?.message || 'Upload failed';

                if (errorMessage.toLowerCase().includes('exceeded the number of videos')) {
                    errorMessage = '🚀 YouTube Quota Reached: Daily upload limit exceeded. Please verify your channel (Phone/ID) in YouTube Studio to lift this limit for global scale.';
                }
